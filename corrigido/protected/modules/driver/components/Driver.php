...
class Driver
{
	...
	public static function jsLang2()
	{
		return array(
    	  ...
    	  "trans_33"=>self::t("No results match"),
    	  "copy_clipboard"=>self::t("copy to clipboard"),
		);
	}
	...
	public static function driverInfoWithRating($driver_id='')
	{		
		$stmt="
		SELECT a.*,
		b.team_name,
	...
	{		
		...
		$order_status=Yii::app()->functions->getOptionAdmin('drv_order_status');	
		 if($json_status = json_decode($order_status,true) ){
	    	$order_status = $json_status;
	    } else {
	    	if(!empty($order_status)){
	    	   $order_status = array($order_status);
	    	} else $order_status = array('pending','paid');
	    }
	    
	    $in_status = '';
	    if(is_array($order_status) && count($order_status)>=1){
	    	foreach ($order_status as $val_status) {
	    		$in_status.=q($val_status).",";
	    	}
		...
		if ( $res=$db->rst($stmt)){
			foreach ($res as $val) {
				...
				if(!empty($val['delivery_time'])){	
					if(getOptionA('time_selection')==1){
						$val['delivery_time'] = substr($val['delivery_time'],0,5);
					}					
	...
	public static function addressToLatLong_OLD($address='')
	{		
		echo 'addressToLatLong';
		die();
	   ...
	   $driver_status='active' , $team_id='', $driver_name='')
	{
		...
		$offline_interval_date = date("Y-m-d H:i:s", strtotime("-6 minutes"));
		
		$online_30_interval_date = date("Y-m-d H:i:s", strtotime("-30 minutes"));
		$online_31_interval_date = date("Y-m-d H:i:s", strtotime("-31 minutes"));
		...
		switch ($stats) {
			case "active":
				...
				if ( $tracking_type==2){										
					$and.=" AND on_duty ='1' \n";
					$and.=" AND CAST(last_login as DATE) BETWEEN ".q($todays_date)." AND ".q($todays_date)." \n";
					$and.=" AND ".q($online_30_interval_date)." < last_login  ";
				} else {					
					$and.=" AND on_duty ='1' \n";
					$and.=" AND CAST(last_login as DATE) BETWEEN ".q($todays_date)." AND ".q($todays_date)." \n";
					$and.=" AND ".q($online_interval_date)." < last_login  ";
				}
				break;
			...
			case "offline":				
				...
				if ( $tracking_type==2){
					$and.=" AND CAST(last_login as DATE) BETWEEN ".q($todays_date)." AND ".q($todays_date)." \n";
					$and.= "AND (on_duty ='2' OR ".q($online_31_interval_date)." > last_login  ) ";
				} else {
					$and.=" AND ".q($offline_interval_date)." > last_login \n";
				}
			default:
		...
		(
		  ...
		  where
		  driver_id=a.driver_id		  		 
		  and CAST(delivery_date as DATE) BETWEEN ".q($transaction_date)." AND ".q($transaction_date)."
		) as total_task
		FROM
		{{driver}} a
		WHERE 1
   ...
   public static function availableLanguages()
	...
	{				
		...
		if ($key_value==1 && $info['enabled_push']==1){
			...
			$push_message=self::smarty('CompanyName',$website_title,$push_message);				
			$params=array(
		...
		if ($key_value==1 && $info['driver_phone']!=""){
		   ...
		   $sms_message=self::smarty('CompanyName',$website_title,$sms_message);		   
		   
		   Yii::app()->db->createCommand()->insert("{{sms_broadcast_details}}",array(
		     'contact_phone'=>isset($info['driver_phone'])?$info['driver_phone']:'',
		     'sms_message'=>$sms_message,
		     'date_created'=>FunctionsV3::dateNow(),
		     'ip_address'=>$_SERVER['REMOTE_ADDR']
		   ));
		   Driver::consumeUrl(FunctionsV3::getHostURL().Yii::app()->createUrl("cron/processsms"));
		}
		...
		if ($key_value==1 && $info['driver_email']!=""){
		   ...
		   $email_message=self::smarty('CompanyName',$website_title,$email_message);	
		   
		   $subject = str_replace("_","",$key);
		   $subject = strtolower($subject);
		   $subject = Driver::t($subject);
		   
		   Yii::app()->db->createCommand()->insert("{{email_logs}}",array(
		      'email_address'=>isset($info['driver_email'])?$info['driver_email']:'',
		      'subject'=>$subject,
		   ...
		   Driver::consumeUrl(FunctionsV3::getHostURL().Yii::app()->createUrl("cron/processemail"));   
		}
	...
	public static function sendNotificationCustomer($key='',$info='')
	{
		...
		if ( $key_is_enabled==1 && $info['order_id']>0){			
			...
			$push_title = strtolower($push_title);
		...
		if ( $key_is_enabled==1 && $info['contact_number']!=""){
			...
			if ( $send_sms= Yii::app()->functions->sendSMS($info['contact_number'],$message)){		   	    
				...
				$params['user_id']= (integer) $info['user_id'];
	...
	$start_date='' , $end_date='' )
	{
		...
		switch ($time) {			
			...
			case "month":	
			    ...
			    $end=date("Y-m-d", strtotime("+1 day"));
				$and.= " AND delivery_date_only BETWEEN '$start' AND '$end' ";
			   break;
			   
			case "custom":		
			   $and.= " AND delivery_date_only BETWEEN '$start_date' AND '$end_date' ";
			   break;
		...
		if ($team>0){			
			...
			$and2.=" AND team_id=".self::q($team)." ";
		}
		if($driver>0){
			$and.=" AND driver_id=".self::q($driver)." ";
			$and2.=" AND driver_id=".self::q($driver)." ";
		}
		...
		if ( $chart_option=="time"){
			$stmt="
			SELECT 
			a.delivery_date_only as delivery_date,
			a.driver_name,
				
			(
			select sum(total_w_tax)
			from {{driver_task_view}}
			where delivery_date_only = a.delivery_date_only
			...
			(
			 ...
			 and delivery_date_only = a.delivery_date_only
			) as successful,
			...
			AND driver_id <>0
			$and
			GROUP BY delivery_date_only,total_order_amount,successful,cancelled,failed
			";
		} else {
			$stmt="
			SELECT 
			...
			AND driver_id <>0	
			$and2
			GROUP BY a.driver_id,total_order_amount,successful,cancelled,failed
			";
		}
    ...
    public static function latToAdress_OLD($lat='' , $lng='')
	{
		echo 'latToAdress';
		die();
    ...
    public static function getTaskDistance_OLD($lat1='',$lon1='', $lat2='',$lon2='',
    $transport_type='')
    {    	 
    	
    	echo 'getTaskDistance';
		die();
   ...
   public static function driverDropDownList($user_type='', $user_id='')
	...
	{		
		...
		if (!empty($date)){			
			$new_interval_date = date("Y-m-d H:i:s", strtotime("-2 minutes"));			
			$and.=" AND ".q($new_interval_date)." < date_created  ";
		}
	...
	public static function checkNewOfflineDriver($user_type='',$user_id='')
	{
		...
		$todays_date=date('Y-m-d');
		$offline_interval_date1 = date("Y-m-d H:i:s", strtotime("-6 minutes"));
		$offline_interval_date2 = date("Y-m-d H:i:s", strtotime("-8 minutes"));
		
		$and.=" AND CAST(last_login as DATE) BETWEEN ".q($todays_date)." AND ".q($todays_date)." \n";
		$and.=" AND ( on_duty='2' OR 
		 ".q($offline_interval_date1)." > last_login  
		 AND
		  ".q($offline_interval_date2)." < last_login 
		 ) ";
	...
	public static function getSingleAppDeviceIDByOrderID($order_id='')
	{		
		
		$table1 = Yii::app()->db->schema->getTable("{{singleapp_device_reg}}");
		$table2 = Yii::app()->db->schema->getTable("{{view_order}}");
		if( $table1 && $table2 ){		
			$stmt="
			SELECT 
			a.order_id,
	...
	public static function getNewTask($driver_id='')
	{
		$todays_date=date('Y-m-d');				
		$less_interval = date("Y-m-d H:i:s", strtotime("-1 minutes"));
		
		$stmt="
		SELECT a.task_id 
		FROM {{driver_task}} a
		WHERE status IN ('assigned')
		AND driver_id=".q( (integer) $driver_id)."
		AND CAST(date_created as DATE) BETWEEN ".q($todays_date)." AND ".q($todays_date)."
		AND (
		  ".q($less_interval)." < date_created  
		  OR
		  ".q($less_interval)." < date_modified  		 		 
		)
		 ...
		 OR task_id IN (
		  ...
		  task_id=a.task_id
		  and
		  driver_id=".self::q(  (integer) $driver_id)."
		  and
		  status='process'
		  and
		  task_status='unassigned'
		  and
		  ".q($less_interval)." < date_process  
		)	
	...
	public static function getAssignmentCount($task_id='')
	{
		$db=new DbExt;
		$stmt="
		SELECT count(*) as total
	...
	public static function sendPushToOtherAssignDriver($driver_id='', $task_id='')
	{
		$push_title = Driver::t("Task updated");
		$push_message = Driver::t("Task updated");
		...
		FROM {{driver_assignment}} a
		LEFT JOIN {{driver}} b
		ON
        a.driver_id = b.driver_id
		
		WHERE a.task_id = ".q($task_id)."
		AND a.driver_id NOT IN (".q($driver_id).")
		";
		if($res = Yii::app()->db->createCommand($stmt)->queryAll()){
			foreach ($res as $val) {
				$params = array(
