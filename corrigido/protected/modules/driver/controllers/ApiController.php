...
{	
	...
	{	
		...
		$accepted_action = array(
		  'getlanguagesettings','getappsettings','uploadprofile','uploadtaskphoto','updatedriverlocation',
		  'updatedriverlocationfailed'
		);
		
		if(in_array($action,$accepted_action)){			
			return true;
		}
		...
		if($app_version>="1.8.0"){
	        ...
	        $api_has_key = trim(getOptionA('driver_api_hash_key'));
	                
	        if($api_has_key!=$api_key){
	        	$this->msg = Driver::t("api hash key is not valid");
	        	$this->output();
	        	return false;
	        }	
		} else {
	...
	private function validateApikey($action='')
	{
		$continue=true;
	    $action=strtolower($action);			   
    	...
    	if(!empty($this->data['username']) && !empty($this->data['password'])){
	    	if ( $res=Driver::driverAppLoginNew($this->data['username'],$this->data['password'])){	
	    		...
	    		$params=array(
	    		  ...
	    		  'device_platform'=>isset($this->data['device_platform'])? trim($this->data['device_platform']) :'Android',
	    		  'app_version'=>isset($this->data['app_version'])?$this->data['app_version']:'',
	    		);	   
	    		$params['device_platform']  = strtolower($params['device_platform']);
	    		...
	    		if ( $db->updateData("{{driver}}",$params,'driver_id',$res['driver_id'])){	    			
	    			...
	    			$topic_id = $res['user_type']."_".$res['user_id'];
    ...
    {    	    	    	
    	...
    	if ( $res=Driver::getTaskByDriverIDWithAssigment($driver_id,$this->data['date'],$task_type)){
    		...
    		$this->msg=Driver::prettyPrice($total_order);
    		 ...
    		 $this->msg= $task_type=="completed"?self::t("No completed task"):self::t("No task for the day");
    		 if($app_version>="1.7.1"){
    		 	$this->details=array(    			  
    			  'todays_date_raw'=>$todays_date_raw,
    			  'todays_date'=>$todays_date,
    	...
    	if (isset($this->data['status_raw']) && isset($this->data['task_id'])){
    		...
    		switch ($this->data['status_raw']) {
    			...
    			case "declined":
    				...
    				if ( $assigment_info=Driver::getAssignmentCount($task_id)){    					
    					$stmt_assign="UPDATE 
    					{{driver_assignment}}
    					...
    					";    					    					
    					Yii::app()->db->createCommand($stmt_assign)->query();
    			...
    			case "acknowledged":    	
    				...
    				/*SEND PUSH TO OTHER DRIVER WITH ASSIGN*/
    				Driver::sendPushToOtherAssignDriver($driver_id,$task_id);
    ...
    {    	
		...
		if(!empty($token)){
			if ( $resp=Driver::getDriverByToken($token)) {
				$topic_id = $resp['user_type']."_".$resp['user_id'];
    ...
    public function actionViewOrderDetails()
    {
		...
		if ($data=FunctionsV3::getReceiptByID($order_id)){			
		 ...
		 $full_merchant_address=$merchant_info['street']." ".$merchant_info['city']. " ".$merchant_info['state']." ".$merchant_info['post_code'];
		         ...
		         if(isset($data['opt_contact_delivery'])){
    ...
    {    	
    	...
    	$driver_id=$token['driver_id'];
    	$params=array(
    	  'location_lat'=>isset($this->data['lat'])?$this->data['lat']:'',
    	  'location_lng'=>isset($this->data['lng'])?$this->data['lng']:'',
    	  'last_login'=>FunctionsV3::dateNow(),
	      'last_online'=>strtotime("now"),
	      'app_version'=>isset($this->data['app_version'])?$this->data['app_version']:''
    	);
    	...
    	if ($up){
    		...
    		if ($is_record==1){
	    		$logs=array(	    		  
	    	      ...
	    	      'date_log'=>date("Y-m-d"),
	    	      'full_request'=>json_encode($this->data),	      
	    		);	    		
    ...
    public function actionupdateDriverLocationFailed()
    {    	
    	Yii::app()->functions->createLogs("failed",'driver_failed');
    	$this->output();   
    }
    ...
    {    	    	
    	$this->msg = Driver::t("not available");
    	$this->output();
    }
