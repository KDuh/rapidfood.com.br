...
class AjaxController extends CController
{
	...
	public function actiondriverList()
	{
		$aColumns = array(
		  ...
		  'team_id','device_id','ratings','status','driver_id'
		);
		$t=AjaxDataTables::AjaxData($aColumns);		
		if (isset($_GET['debug'])){
		    dump($t);
	...
	{		
		if(isset($this->data['id'])){
			if ( $res=Driver::driverInfoWithRating($this->data['id'])){				 
				 $this->code=1;
				 $this->msg=Driver::t("Successful");
				 $this->details=$res;				 
			} else $this->msg=Driver::t("Record not found");
	...
	public function actionassignTask()
	{
		...
		$task_id = (integer) isset($this->data['task_id'])?$this->data['task_id']:0;				
		...
		if($Validator->validate()){
			...
			$res=Driver::getTaskId( (integer) $this->data['task_id'] );
						...
						if ( $json_details !=false){
						    Yii::app()->functions->displayOrderHTML(array(
						     ...
						     $order_details_head.="<table class=\"table table-striped\">";
						      ...
						      $order_details_head.=Driver::receiptRow("TRN Type",t($data['trans_type']));
						      $order_details_head.=Driver::receiptRow("Payment Type",(t($data['payment_type'])));
		                      ...
		                      if(isset($data['opt_contact_delivery'])){
		                      	  if($data['opt_contact_delivery']==1){
		                      	    $order_details_head.=Driver::receiptRow(t("Delivery options"),t("Leave order at the door or gate"));
		                      	  }
		                      }						
		...
		if ( $res=$DbExt->rst($stmt)){
			...
			foreach ($res as $val) {
			    ...
			    $feed_data['aaData'][]=array(
			      ...
			      $date_created,
			      normalPrettyPrice($val['rating']),
			      "<div style=\"margin-bottom:10px;\">$status</div>$action",			      
			    );			    
			}
			if (isset($_GET['debug'])){
			   dump($feed_data);
	...
	{		
	    ...
	    isset($this->data['drv_order_status'])?json_encode($this->data['drv_order_status']):'');
		...
		if ( $res=$DbExt->rst($stmt)){
			...
			foreach ($res as $val) {
			    ...
			    $feed_data['aaData'][]=array(
			      ...
			      $date_created,
			      $status."<br>".'<small>'.$val['json_response'].'</small>',
			    );			    
			}
			if (isset($_GET['debug'])){
			   dump($feed_data);
	...
	{			
		...
		if (is_array($data) && count($data)>=1 && $this->data['chart_type_option']=="time"){			
	...
	{		
		...
		if ( self::$db->insertData("{{driver_bulk_push}}", $params)){
			...
			Driver::consumeUrl(FunctionsV3::getHostURL().Yii::app()->createUrl("driver/cron/processbulk"));
    ...
    {    	
    	...
    	if ( $res=Driver::getTaskId($this->data['task_id'])){
			...
			if ( $res2=$db->rst($stmt)){				
				...
				$driver_assign_request_expire = (integer) Driver::getOption('driver_assign_request_expire',
		...
		if ( $res=$DbExt->rst($stmt)){
			...
			foreach ($res as $val) {
			    ...
			    $feed_data['aaData'][]=array(
			      ...
			      $val['push_message'],			     			      
			      $date_created,			      
			      $status."<br>".'<small>'.$val['fcm_response'].'</small>',		      
			    );			    
			}
			if (isset($_GET['debug'])){
			   dump($feed_data);
	...
	public function actionsaveMapSettings()
	{
	    ...
	    if(!empty($this->data['drv_default_location'])){
	       ...
	       if ( $res=MapsWrapper::geoCodeAdress($country_name))	{	       	   
	...
	public function actionsaveFCMSettings()
	{
	    ...
	    Yii::app()->functions->updateOptionAdmin("drv_services_json_account",
	    isset($this->data['drv_services_json_account'])?$this->data['drv_services_json_account']:'');
	...
	public function actionuploadjsonAccount()
	{
	    require_once('Uploader.php');
		$path_to_upload=Driver::certificatePath();
