	 ...
	 success: function (data) {	  
	 	...
	 	if (data.code==1){
	 		...
	 		switch (action)
	 		{
	 			...
	 			$(".raty-stars").attr("data-score", data.details.ratings );
...
{	 
	 ...
	 complete: function(data) {		
	 	notification_handle = setInterval(function(){getNotifications()}, 30000); 	
	 },
	 success: function (data) {	  		 	
	 	if (data.code==1){	 	
	 		$(".no-noti").remove();	
...
$(document).ready(function(){
	...
	if ( $("#upload_account_json").exists()) {
	   var uploader = new ss.SimpleUpload({
	       button: 'upload_account_json', // HTML element used as upload button
	       url: ajax_url+"/uploadjsonAccount", // URL of server-side upload handler
	       name: 'uploadfile', // Parameter name of the uploaded file
	       responseType: 'json',
		   ...
		   onComplete: function(filename, response) {	   	  
		   	  ...
		   	  if (response.code==1){	   	  	 	   	  	 
		   	  	 nAlert(response.msg,"success");
		   	  	 $(".drv_services_json_account").remove();
		   	  	 $("form").append( '<input type="hidden" name="drv_services_json_account" class="drv_services_json_account" value="'+ filename +'">' )
		   	  } else {
		   	  	 nAlert(response.msg,"warning");	   	  	 
		   	  }
		   }
   ...
   $( document ).on( "click", ".copy_text", function() {
		...
		nAlert( js_lang.copy_clipboard ,"success");
	});
